...
Per-package configuration options for individual modules. This option is a `map` structure, where the package name to be configured serves as the `key`, and the package-specific configuration serves as the `value`. Currently configurable options include:

- `output-type`: The package compilation artifact type. Its values are the same as those of [`output-type`](#output-type).
- `compile-option`: Additional compilation options used only for this package.
- `combine-all-deps`: The project-level build artifact consolidation switch. Its value is `true` or `false`, and it can be configured only for the `root` package.

All these options are optional and can be configured as needed. As shown below, the output type of the `demo.aoo` package in the `demo` module will be specified as a dynamic library, and the `-g` command will be passed through to the `demo.aoo` package during compilation.
...
Setting `combine-all-deps = true` enables project-level build artifact consolidation. This configuration is the switch for project-level `lto-combine`. For related concepts and configuration combinations, see [Compilation Optimization and Build Artifact Consolidation](#compilation-optimization-and-build-artifact-consolidation). This configuration takes effect only when the following conditions are met:

- Module-level artifact consolidation (`profile.build.combined`) and `LTO` compilation optimization (`profile.build.lto`) are enabled (see [the `profile.build` field](#profilebuild)).
- The configured module is the module corresponding to the currently executed `cjpm build` command, and the configured package is the `root` package of that module. This field is ignored if it is configured in a non-`root` package of the current module or in a dependency module.

When these conditions are met, the module is compiled as follows:

- Except for the `root` package of the module currently being built, every source package participating in the build generates a corresponding `.bc` file in `LTO` optimization mode. These source packages include all subpackages of the current module, as well as the `root` packages and subpackages of source modules on which the current module directly or indirectly depends. Macro packages, test packages, and dependencies that provide only binary artifacts do not generate such `.bc` files.
- On platforms other than iOS, the module's `root` package uses all `.bc` files generated by these source packages as link inputs and produces a dynamic library, even if the packages are not explicitly imported by the `root` package.
- On iOS, the module's `root` package uses all `.bc` files generated by these source packages as link inputs and produces a static library (`.a`).
...
```text
[profile.build]
lto = "full"  # Whether to enable `LTO` (Link Time Optimization) compilation mode. This setting has the same effect as the `level` configuration option of `[profile.build.lto]`. This string-type configuration will be deprecated in future versions. Please use the `level` option under `[profile.build.lto]` to enable LTO.
...
[profile.build.lto]
level = "full" # Whether to enable `LTO` (Link Time Optimization) compilation mode.
...
demo = "dynamic" # Compile the entire module into a dynamic library on non-iOS platforms; configure this as "static" on iOS.
```
...
For `lto` configuration combinations, see [Compilation Optimization and Build Artifact Consolidation](#compilation-optimization-and-build-artifact-consolidation).
...
The `combined` configuration is a key-value pair where the key is the module name (`package.name`). Its value is `dynamic` on non-iOS platforms. When using `lto-combine` on iOS, its value must be `static`. Before configuring this, the module compiles each package into separate dynamic or static library files based on `package.output-type`. After configuration, the module's compilation method changes to:

- On non-iOS platforms, subpackages other than `root` are compiled as static libraries.
- On non-iOS platforms, the `root` package is compiled as a dynamic library and links all subpackage static libraries, regardless of whether the subpackages are dependencies of the `root` package. When other modules depend on this dynamic library as a binary dependency, they can use all symbols from the subpackages.
- On iOS, the `root` package is compiled as a static library (`.a`) and combines the intermediate artifacts of all subpackages through LTO.
...
> **Note:**
>
> - When applying this configuration, compiling the `root` package requires all its subpackages, so ensure the `root` package is not directly or indirectly imported by its subpackages.
...
##### Compilation Optimization and Build Artifact Consolidation

The following three concepts represent an optimization method, a module artifact consolidation configuration, and the build mode formed by combining them:

- `LTO`: Link Time Optimization. When enabled, `cjpm` performs cross-module optimization in `full` or `thin` mode, but it does not automatically output the entire project as a single library.
- `combine`: Module-level artifact consolidation enabled by `[profile.build.combined]`. On non-iOS platforms that support using `combine` independently, subpackages are first built as static libraries and then linked into the `root` package. On iOS, `combine` must be used together with LTO.
- `lto-combine`: The build mode enabled when both `LTO` and `combine` are enabled. It is not a separate configuration item.
  - Module-level `lto-combine`: Enabled jointly by `[profile.build.lto]` and `[profile.build.combined]`.
  - Project-level `lto-combine`: In addition to module-level `lto-combine`, set `combine-all-deps = true` for the current module's `root` package to combine the current project and all its direct and indirect source dependencies.

The build results of the configuration combinations on different platforms are as follows:

| Platform | Configuration Combination | Build Mode | `root` Package Artifact | Other Package or Dependency Artifacts |
| :-- | :-- | :-- | :-- | :-- |
| `Linux/OpenHarmony/Android/iOS` | Configure only `profile.build.lto` | LTO | Generated according to `package.output-type` | Each package is compiled separately with LTO optimization |
| `Linux/OpenHarmony/Android/Windows` | Configure only `profile.build.combined` | Module-level `combine` | Dynamic library | Subpackages are compiled as static libraries and linked into the `root` package |
| `Linux/OpenHarmony/Android` | Configure both `profile.build.lto` and `profile.build.combined` | Module-level `lto-combine` | Dynamic library | All subpackages first generate LTO intermediate artifacts, which are then combined by the `root` package |
| `iOS` | Configure both `profile.build.lto` and `profile.build.combined` | Module-level `lto-combine` | Static library | All subpackages first generate LTO intermediate artifacts, which are then combined by the `root` package |
| `Linux/OpenHarmony/Android` | Additionally set `combine-all-deps = true` | Project-level `lto-combine` | Dynamic library | All `.bc` files generated by source packages in the current build graph, except the current module's `root` package, are combined |
| `iOS` | Additionally set `combine-all-deps = true` | Project-level `lto-combine` | Static library | All `.bc` files generated by source packages in the current build graph, except the current module's `root` package, are combined |

> **Note:**
>
> iOS does not support using `combine` independently. When using module-level or project-level `lto-combine` on iOS, the entry module's value in `profile.build.combined` must be set to `static`.
