...
Starting with Cangjie 1.0.0, the first LTS release, the Cangjie compiler guarantees backward compatibility for cjo files: newer compiler versions can correctly handle cjo files generated by compilers from version 1.0.0 through the current version, but they are not guaranteed to read cjo files generated by later compiler versions. Before the LTS release, cjo files generated by different compiler versions may have compatibility issues. If the compiler encounters an incompatible cjo file, it reports an error and stops compilation. In this case, upgrade the SDK or rebuild the relevant dependencies with the current compiler version.
