...
| <!--DelRow--> Constructor   | `init(causedBy: Exception)` Constructor that allows setting a cause exception.               |
| Constructor   | `init(message: String)` Constructor that allows setting an exception message.               |
| <!--DelRow--> Constructor   | `init(message: String, causedBy: Exception)` Constructor that allows setting an exception message and a cause exception.               |
| <!--DelRow--> Property      | `mut prop causedBy: ?Exception` Allows setting a cause exception and returns the cause exception. |
