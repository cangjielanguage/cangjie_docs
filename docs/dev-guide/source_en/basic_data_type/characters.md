...
`Rune` can be converted to `UInt32`, and integer types can be converted to `Rune`. For specific type conversion syntax and rules, please refer to [Conversion from `Rune` to `UInt32` and from Integer Types to `Rune`](./integer.md#conversion-from-rune-to-uint32-and-from-integer-types-to-rune).