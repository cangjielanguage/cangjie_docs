...
仓颉编译器自 1.0.0（首个 LTS 版本）开始承诺 cjo 文件的向后兼容性：后续版本的编译器能够正确处理由 1.0.0 至当前版本的编译器生成的 cjo 文件，但不保证能够读取更高版本编译器生成的 cjo 文件。在 LTS 版本之前，不同版本编译器生成的 cjo 文件可能存在兼容性问题。若读取到不兼容的 cjo 文件，编译器会报错并停止编译；此时可升级 SDK，或使用当前版本的编译器重新编译相应依赖。
