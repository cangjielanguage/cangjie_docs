...
下述示例中，两个变量均为函数类型且函数参数类型不同，但由于它们不是函数声明所以不能重载，编译会抛出重定义错误：
...
下述示例中，变量 `f` 为函数类型，但变量与函数不允许重名，代码编译会触发重定义错误：
...
下述示例中，静态成员函数 `f` 与实例成员函数 `f` 参数类型不同，但类内静态成员函数与实例成员函数不支持重载，代码编译会报错：
